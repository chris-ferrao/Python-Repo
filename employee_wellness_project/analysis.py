...
# Apply a cohesive Plotly template for consistent, aesthetic charts
pio.templates['wellness_clean'] = go.layout.Template(
    layout=go.Layout(
        font=dict(family='Inter, Arial, sans-serif', size=13, color='#0f172a'),
        colorway=['#4f46e5', '#22c55e', '#f59e0b', '#0ea5e9', '#ef4444', '#64748b'],
...
pio.templates.default = 'wellness_clean'
...
# -------------------------------------------------------------------- #
# --- SUMMARY: ADVANCED ANALYTICS FOR STAKEHOLDERS ---
# -------------------------------------------------------------------- #

def get_kpi_uplift_from_benefits() -> str:
    """Absolute uplift in treatment rate for employees with benefits vs without."""
    ct = pd.crosstab(df['benefits'], df['treatment'], normalize='index').mul(100)
    yes_rate = ct.loc['Yes', 'Yes'] if 'Yes' in ct.index else 0.0
    no_rate = ct.loc['No', 'Yes'] if 'No' in ct.index else 0.0
    uplift = max(0.0, yes_rate - no_rate)
    return f"+{uplift:.1f} pp"

def get_kpi_remote_share() -> str:
    """Share of workforce working remotely (Yes)."""
    share = df['remote_work'].value_counts(normalize=True).get('Yes', 0) * 100
    return f"{share:.1f}%"

def plot_heatmap_treatment_by_size_and_interference() -> go.Figure:
    """Heatmap of treatment rate by company size and work_interfere (stakeholder-friendly)."""
    rate = pd.crosstab(df['no_employees'], df['work_interfere'], df['treatment'] == 'Yes', aggfunc='mean').mul(100)
    rate = rate.reindex(index=['5-Jan','25-Jun','26-100','100-500','500-1000','More than 1000'])
    fig = px.imshow(
        rate,
        aspect='auto',
        color_continuous_scale='Blues',
        labels=dict(color='Treatment Rate (%)'),
        title='Treatment Rate Heatmap: Company Size × Work Interference'
    )
    return fig

def plot_grouped_bar_gender_x_treatment() -> go.Figure:
    """Grouped bars: treatment rate by gender across interference!=Never."""
    affected = df[df['work_interfere'] != 'Never']
    rate = pd.crosstab(affected['Gender'], affected['treatment'], normalize='index').mul(100).reset_index()
    fig = px.bar(rate, x='Gender', y='Yes', color='Gender', title='Treatment Rate by Gender (Affected by Work Interference)')
    return fig

def plot_donut_remote_vs_treatment() -> go.Figure:
    """Donut: overall share of treatments segmented by remote vs onsite."""
    seg = df.groupby('remote_work')['treatment'].value_counts().unstack(fill_value=0)
    # Focus on treatment counts
    values = seg['Yes'] if 'Yes' in seg.columns else seg.sum(axis=1)
    fig = px.pie(values=values.values, names=values.index, hole=0.55, title='Where Are Treatments Coming From? (Remote vs Onsite)')
    return fig

def plot_leave_accessibility_distribution() -> go.Figure:
    """Stacked bars: distribution of leave accessibility response by company size."""
    dist = df.groupby('no_employees')['leave'].value_counts(normalize=True).mul(100).rename('percentage').reset_index()
    fig = px.bar(
        dist, x='no_employees', y='percentage', color='leave', barmode='stack',
        title='Distribution of Leave Accessibility by Company Size',
        category_orders={'no_employees': ['5-Jan','25-Jun','26-100','100-500','500-1000','More than 1000']}
    )
    return fig
